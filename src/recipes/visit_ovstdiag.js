...
function sourceSql(from, to) {
  ...
  if (from && to) {
    params.push(from, to);
    // เทียบแบบช่วง (sargable) ให้ใช้ index บน visit_begin_visit_time ได้
    where = "WHERE v.visit_begin_visit_time >= $1::date AND v.visit_begin_visit_time < ($2::date + interval '1 day')";
  }
