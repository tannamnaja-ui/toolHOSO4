...
/* ============================================================
   ...
   ต้นทาง: PostgreSQL (t_order + t_visit + t_order_drug)
...
function sourceSql(from, to) {
  ...
  if (from && to) {
    params.push(from, to);
    // sargable: เทียบ order_date_time ตรง ๆ ทั้งช่วง ค.ศ. และ พ.ศ. (+543 ปี) — ใช้ index ได้
    dateFilter =
...
SELECT
    ...
    TO_CHAR(v.visit_begin_visit_time, 'HH24:MI:SS') AS vsttime,
    ...
    CASE WHEN v.f_visit_type_id = '1' THEN o.t_order_id ELSE NULL END AS order_no,
...
module.exports = {
  ...
  lookups: {
    ...
    iptOrderNo:   { table: 'ipt_order_no',  match: 'oldcode',             ret: 'order_no' },
  ...
  columns: [
    ...
    { col: 'order_no',      field: 'order_no', lookup: 'iptOrderNo' },      // null -> ไม่ lookup
